"""第30课数据读取与描述统计示例；请据分析任务扩展。"""
from pathlib import Path
import csv,json,statistics
ROOT=Path(__file__).resolve().parents[1]
LESSON=ROOT/"lesson-30"
DATA=ROOT/"data"/"policy"/"weekly_demand.csv"
def main():
    ...
    for group in sorted({row["holiday_known"] for row in rows}):
        subset=[row for row in rows if row["holiday_known"]==group]
        stats={"n":len(subset)}
        for field in ["demand","available_capacity"]:
