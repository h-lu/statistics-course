"""第27课数据读取与描述统计示例；请据分析任务扩展。"""
from pathlib import Path
import csv,json,statistics
ROOT=Path(__file__).resolve().parents[1]
LESSON=ROOT/"lesson-27"
DATA=ROOT/"data"/"policy"/"support_observational.csv"
def main():
    ...
    for group in sorted({row["support"] for row in rows}):
        subset=[row for row in rows if row["support"]==group]
        stats={"n":len(subset)}
        for field in ["severity","baseline_burden","followup_burden"]:
