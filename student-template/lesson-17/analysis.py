...
FILES=["paired_study.csv","independent_comparison.csv","repeated_measurements.csv"]
def main():
    summaries=[]
    for filename in FILES:
        path=ROOT/"data"/"experiments"/filename
        ...
        summaries.append({"file":"data/experiments/"+filename,"rows":len(rows),"columns":columns,"missing":{c:sum(r[c]=="" for r in rows) for c in columns},"pooled_numeric_description":numeric})
    ...
    print("已读取数据；示例统计结果位于 lesson-17/artifacts/starting_inventory.json。请继续完成本课分析任务。")
