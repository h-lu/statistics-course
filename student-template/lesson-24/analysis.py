"""第24课数据读取与描述统计示例；请据分析任务扩展。"""
from pathlib import Path
import csv,json,statistics
ROOT=Path(__file__).resolve().parents[1]
LESSON=ROOT/"lesson-24"
...
def main():
    ...
    for group in sorted({row["month"] for row in rows}):
        subset=[row for row in rows if row["month"]==group]
        stats={"n":len(subset)}
        for field in ["escalated","resolution_minutes"]:
